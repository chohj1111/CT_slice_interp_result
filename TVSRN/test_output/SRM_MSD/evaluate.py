...
for case_name in os.listdir(output_dir):
    ...
    y_denorm = nib.load(os.path.join(gt_dir, case, 'imaging_1mm.nii.gz')).get_fdata()
    y_pre_denorm = nib.load(os.path.join(output_dir, case_name)).get_fdata()
    
    y_denorm = np.rot90(y_denorm)
    y_pre_denorm = np.rot90(y_pre_denorm)
     
    last_slice = y_denorm.shape[-1] - (y_denorm.shape[-1] - 1) % 5
    ...
    psnr /= y_denorm_norm.shape[-1]
    ssim /= y_denorm_norm.shape[-1]                        
